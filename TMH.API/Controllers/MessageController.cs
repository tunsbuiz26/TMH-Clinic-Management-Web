...
namespace TMH.API.Controllers
{
    ...
    public class MessageController : ControllerBase
    {
        ...
        public MessageController(MessageService msg, IHubContext<ChatHub> hub)
        {
            _msg = msg;
            _hub = hub;
        }
        ...
        public async Task<IActionResult> SendMessage(int id, [FromBody] SendMessageDto dto)
        {
            ...
            await _hub.Clients.Group($"conv_{id}").SendAsync("ReceiveMessage", msg);
            if (await _msg.IsPatientStaffConvAsync(id))
                await _hub.Clients.Group("staff_inbox").SendAsync("ReceiveMessage", msg);
