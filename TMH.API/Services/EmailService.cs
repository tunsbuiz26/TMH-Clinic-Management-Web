...
namespace TMH.API.Services
{
    ...
    public class EmailService
    {
        ...
        {
            var subject = "🔑 Đặt lại mật khẩu — Phòng Khám TMH";
            var body = PasswordResetHtml(toName, resetLink);
            return SendAsync(toEmail, toName, subject, body);
        }
        ...
        // ── Gửi email tiếp nhận lịch ChoPhanCong ─────────────────────────
        public Task SendPendingBookingNoticeAsync(
            string toEmail,
            string toName,
            string bookingCode,
            ...
            string? note)
        {
            ...
            string preferredInfo = preferredDate.HasValue
                ? $"<p><strong>Ngày mong muốn khám:</strong> {preferredDate.Value:dd/MM/yyyy}</p>"
                : "<p>Bạn chưa chọn ngày cụ thể — lễ tân sẽ liên hệ để sắp xếp.</p>";

            string noteInfo = !string.IsNullOrWhiteSpace(note)
                ? $"<p><strong>Ghi chú:</strong> {note}</p>"
                : string.Empty;

            var subject = $"[TMH] Tiếp nhận yêu cầu đặt lịch – {bookingCode}";
            var body = WrapLayout($@"
<div class='greeting'>Xin chào {toName},</div>
<p style='font-size:13px;color:#555;line-height:1.7;margin-bottom:16px'>
  Chúng tôi đã nhận được yêu cầu đặt lịch khám cho hồ sơ <strong>{patientName}</strong>.
</p>

<div class='info-box'>
  <div class='info-row'><span class='info-lbl'>📋 Mã lịch hẹn</span><span class='info-val'>{bookingCode}</span></div>
  <div class='info-row'><span class='info-lbl'>📌 Trạng thái</span><span class='info-val'>Chờ phân công</span></div>
</div>
...
<p style='font-size:13px;color:#555;line-height:1.7;margin-top:16px'>
  Lễ tân sẽ liên hệ và thông báo lịch khám cụ thể sớm nhất có thể.<br/>
  Bạn cũng sẽ nhận được email xác nhận khi lịch được xếp.
</p>
...
<div class='cta'>
  <a class='btn' href='http://localhost:63355/Booking/MyAppointments'>Xem lịch khám của tôi</a>
        ...
        private static string PasswordResetHtml(string toName, string resetLink) => WrapLayout($@"
<div class='greeting'>Xin chào {toName},</div>
<p style='font-size:13px;color:#555;line-height:1.7;margin-bottom:16px'>
  Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản của bạn.<br>
  Nhấn vào nút bên dưới để tạo mật khẩu mới. Liên kết có hiệu lực trong <strong>1 giờ</strong>.
</p>
...
<p style='font-size:11px;color:#9ab0bf;margin-top:20px;line-height:1.7'>
  Nếu bạn không yêu cầu đặt lại mật khẩu, hãy bỏ qua email này. Tài khoản của bạn vẫn an toàn.<br>
  Liên kết sẽ hết hạn sau 1 giờ kể từ khi email này được gửi.
</p>");
