...
function loadAllArticles() {
    ...
    container.innerHTML = '<div class="loading-row">Đang tải...</div>';
    fetch('/Admin/GetAllArticles')
        .then(function (r) { return r.json(); })
        .then(function (data) {
            var filterEl = document.getElementById('artFilterStatus');
            var filter   = filterEl ? filterEl.value : '';
            if (filter === 'pub')     data = data.filter(function(a){ return (a.statusDisplay || a.StatusDisplay || '') === 'Đã đăng'; });
            if (filter === 'pending') data = data.filter(function(a){ return (a.statusDisplay || a.StatusDisplay || '') === 'Chờ duyệt'; });
            if (filter === 'draft')   data = data.filter(function(a){ return (a.statusDisplay || a.StatusDisplay || '') === 'Nháp'; });

            if (!data.length) {
                container.innerHTML = '<div class="loading-row" style="padding:32px;text-align:center;">Không có bài viết nào.</div>';
                return;
            }
            container.innerHTML = data.map(function (a) {
                var id    = a.id    || a.Id;
                var title = a.title || a.Title || '';
                var author= a.authorName || a.AuthorName || '';
                var cat   = a.category   || a.Category   || '';
                var statusDisplay = a.statusDisplay || a.StatusDisplay || '';
                var isPub     = statusDisplay === 'Đã đăng';
                var isPending = statusDisplay === 'Chờ duyệt';
                var date  = new Date(a.createdAt || a.CreatedAt).toLocaleDateString('vi-VN');
                var badge = isPub
                    ? '<span class="adm-badge-pub">Đã đăng</span>'
                    : isPending
                    ? '<span class="adm-badge-draft" style="background:#fff3cd;color:#856404;border-color:#ffc107;">Chờ duyệt</span>'
                    : '<span class="adm-badge-draft">Nháp</span>';
                var initials = author.trim().split(' ').map(function(w){ return w[0]; }).slice(-2).join('').toUpperCase();
                return '<div class="adm-article-card">'
                    + '<div style="flex:1;min-width:0;">'
                    +   '<div class="adm-article-card-title" title="' + title + '">' + title + '</div>'
                    +   '<div class="adm-article-card-meta">' + cat + ' · ' + date + ' · ' + badge + '</div>'
                    + '</div>'
                    + '<div class="adm-art-author" title="' + author + '">'
                    +   '<div class="adm-art-avatar">' + initials + '</div>'
                    +   '<span style="font-size:12px;color:#3a5a72;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100px;">' + author + '</span>'
                    + '</div>'
                    + '<div class="adm-art-actions">'
                    +   '<button class="btn-sm" onclick="openAdminEditor(' + id + ')">Sửa</button>'
                    +   '<button class="btn-sm ' + (isPub ? 'unpub' : 'pub') + '" onclick="toggleArticle(' + id + ',this)">' + (isPub ? 'Gỡ bài' : isPending ? 'Duyệt' : 'Duyệt đăng') + '</button>'
                    +   '<button class="btn-sm danger" onclick="adminDeleteArticle(' + id + ')">Xóa</button>'
                    + '</div>'
                    + '</div>';
            }).join('');
        })
        .catch(function () { document.getElementById('articlesTable').innerHTML = '<div class="loading-row">Lỗi kết nối.</div>'; });
}
...
// ══════════════════════════════════════════════════════════
var _admCurArticleId = null;

function openAdminEditor(id) {
    _admCurArticleId = id;
    ['admEditorErr','admEditorOk','admUploadErr'].forEach(function(i){ document.getElementById(i).style.display='none'; });
    document.getElementById('admEditorTitle').textContent = id ? 'Sửa bài viết' : 'Viết bài mới';
    ['admArtTitle','admArtSummary','admArtContent','admArtThumbnail'].forEach(function(f){ document.getElementById(f).value=''; });
    document.getElementById('admArtCategory').value = '';
    document.getElementById('admArtImageFile').value = '';
    ...
    fetch('/Admin/AdminGetDoctors')
        .then(function(r){ return r.json(); })
        .then(function(docs) {
            var sel = document.getElementById('admArtDoctorId');
            sel.innerHTML = '<option value="">-- Chọn tác giả --</option>' +
                docs.map(function(d){ return '<option value="' + (d.id||d.Id) + '">' + (d.fullName||d.FullName||d.name||'') + '</option>'; }).join('');
        });

    if (id) {
        fetch('/Admin/AdminGetArticleDetail?id=' + id)
            .then(function(r){ return r.json(); })
            .then(function(a) {
                document.getElementById('admArtTitle').value     = a.title     || '';
                document.getElementById('admArtCategory').value  = a.category  || '';
                document.getElementById('admArtSummary').value   = a.summary   || '';
                document.getElementById('admArtContent').value   = a.content   || '';
                document.getElementById('admArtThumbnail').value = a.thumbnail || '';
                if (a.doctorId) document.getElementById('admArtDoctorId').value = a.doctorId;
                if (a.thumbnail) {
                    document.getElementById('admThumbPreview').src = a.thumbnail;
    ...
    document.getElementById('admEditorOverlay').classList.add('open');
}

function closeAdminEditor() {
    document.getElementById('admEditorOverlay').classList.remove('open');
}

function admPreviewUpload(input) {
    var file = input.files[0];
    if (!file) return;
    var errEl = document.getElementById('admUploadErr');
    errEl.style.display = 'none';
    if (!file.type.startsWith('image/')) { errEl.textContent = 'Chỉ chấp nhận file ảnh.'; errEl.style.display=''; return; }
    if (file.size > 5*1024*1024) { errEl.textContent = 'Ảnh tối đa 5MB.'; errEl.style.display=''; return; }

    var reader = new FileReader();
    reader.onload = function(e) {
        document.getElementById('admThumbPreview').src = e.target.result;
        document.getElementById('admThumbPreview').style.display = '';
        document.getElementById('admThumbPlaceholder').style.display = 'none';
    };
    ...
    var fd = new FormData();
    fd.append('file', file);
    fetch('/Admin/AdminUploadArticleImage', { method: 'POST', body: fd })
        .then(function(r){ return r.json(); })
        .then(function(d) {
            if (d.success) { document.getElementById('admArtThumbnail').value = d.url; }
            else { errEl.textContent = d.message || 'Upload thất bại.'; errEl.style.display=''; }
        })
        .catch(function(){ errEl.textContent = 'Lỗi upload ảnh.'; errEl.style.display=''; });
}
...
function saveAdminArticle(publish) {
    ...
    if (!_admCurArticleId && !doctorId) { errEl.textContent='Vui lòng chọn tác giả.'; errEl.style.display=''; return; }

    var payload = {
        title:    title,
        category: category,
        summary:  document.getElementById('admArtSummary').value.trim(),
        content:  document.getElementById('admArtContent').value.trim(),
        thumbnail:document.getElementById('admArtThumbnail').value.trim() || null,
        publish:  publish,
        doctorId: doctorId ? parseInt(doctorId) : null
    };

    var url = _admCurArticleId
        ? '/Admin/AdminUpdateArticle?id=' + _admCurArticleId
        : '/Admin/AdminCreateArticle';
    ...
    .then(function(d) {
        if (d.success || d.Success) {
            document.getElementById('admEditorOk').textContent = d.message || 'Lưu thành công!';
            document.getElementById('admEditorOk').style.display = '';
            tabLoaded['articles'] = false;
            loadAllArticles();
            setTimeout(closeAdminEditor, 1200);
        } else {
            errEl.textContent = d.message || d.Message || 'Lưu thất bại.';
            errEl.style.display = '';
        }
    })
    .catch(function(){ errEl.textContent='Lỗi kết nối.'; errEl.style.display=''; });
}
