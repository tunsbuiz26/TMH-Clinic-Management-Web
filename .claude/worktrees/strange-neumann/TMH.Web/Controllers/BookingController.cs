...
namespace TMH.Web.Controllers
{
    public class BookingController : Controller
    {
        ...
        // GET /Booking/GetPastDoctorsJson — trả về danh sách bác sĩ đã từng khám (distinct, hoàn thành)
        [HttpGet]
        public async Task<IActionResult> GetPastDoctorsJson()
        {
            ...
            // Lấy bác sĩ từ lịch đã hoàn thành, distinct theo tên bác sĩ
            var pastDoctors = appointments
                .Where(a => a.StatusDisplay == "Hoàn thành")
                .GroupBy(a => a.DoctorName)
                .Select(g => new { doctorName = g.Key, specialty = g.First().Specialty })
                .ToList();

            return Json(pastDoctors);
        }
        ...
        // POST /Booking/CancelAjax/{id} — huỷ lịch kèm lý do (AJAX)
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CancelAjax(int id, [FromBody] CancelAppointmentDto dto)
        {
            if (HttpContext.Session.GetString("JwtToken") == null)
                return Json(new { success = false, message = "Vui lòng đăng nhập lại." });
            var result = await _api.CancelAppointmentAsync(id, dto?.CancellationReason);
            return Json(new { success = result?.Success == true, message = result?.Message ?? "Có lỗi xảy ra." });
        }

        // POST /Booking/SubmitReview — bệnh nhân gửi đánh giá sau khám (AJAX)
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> SubmitReview([FromBody] SubmitReviewDto dto)
        {
            if (HttpContext.Session.GetString("JwtToken") == null)
                return Json(new { success = false, message = "Vui lòng đăng nhập lại." });
            var result = await _api.SubmitReviewAsync(dto);
            return Json(new { success = result?.Success == true, message = result?.Message ?? "Có lỗi xảy ra." });
        }
