...
namespace TMH.Web.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        // ── Bài viết (Admin soạn / sửa) ──────────────────────────────
        [HttpGet]
        public async Task<IActionResult> AdminGetArticleDetail(int id)
        {
            if (!IsAdmin()) return Unauthorized();
            var raw = await _api.GetRawJsonAsync($"api/article/admin/{id}");
            return Content(raw ?? @"{}", "application/json");
        }

        [HttpGet]
        public async Task<IActionResult> AdminGetDoctors()
        {
            if (!IsAdmin()) return Unauthorized();
            var raw = await _api.GetRawJsonAsync("api/admin/doctors");
            return Content(raw ?? "[]", "application/json");
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public async Task<IActionResult> AdminUploadArticleImage()
        {
            if (!IsAdmin()) return Unauthorized();
            var file = Request.Form.Files.FirstOrDefault();
            if (file == null) return BadRequest(new { success = false, message = "Không có file." });
