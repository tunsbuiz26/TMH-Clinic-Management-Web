...
namespace TMH.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [AllowAnonymous]
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDto dto, [FromServices] EmailService emailSvc)
        {
            if (!ModelState.IsValid)
                return BadRequest(new { success = false, message = "Email không hợp lệ." });

            var result = await _authService.ForgotPasswordAsync(dto, emailSvc);
            return Ok(result);
        }
