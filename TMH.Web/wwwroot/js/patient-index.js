...
function norm(a) {
    return {
        ...
        paymentMethod : a.paymentMethod  != null ? a.paymentMethod  : (a.PaymentMethod  || null),
        paymentStatus : a.paymentStatus  != null ? a.paymentStatus  : (a.PaymentStatus  || null)
    };
}
...
function showToast(msg, ok) {
    ...
    var countdown= document.getElementById('toastCountdown');

    // Fallback nếu toast chưa có structure mới
    if (!icon) { t.textContent = msg; t.className = 'toast ' + (ok ? 'ok' : 'err'); t.style.display = 'block'; clearTimeout(t._timer); t._timer = setTimeout(function(){ t.style.display='none'; }, 3500); return; }

    clearTimeout(_toastTimer);
    clearInterval(_toastCountTimer);

    t.className = 'toast ' + (ok ? 'ok' : 'err');
    icon.textContent  = ok ? '✅' : '❌';
    title.textContent = ok ? 'Thành công' : 'Có lỗi xảy ra';
    ...
    countdown.textContent = 'Tự đóng sau 3s';
    ...
    var secs = 3;
    _toastCountTimer = setInterval(function() {
        secs--;
        if (secs > 0) {
            countdown.textContent = 'Tự đóng sau ' + secs + 's';
        } else {
            clearInterval(_toastCountTimer);
        }
    }, 1000);
...
function closeToast() {
    ...
    clearInterval(_toastCountTimer);
    var t = document.getElementById('toast');
    if (t) { t.style.opacity = '0'; t.style.transition = 'opacity .25s'; setTimeout(function(){ t.style.display='none'; t.style.opacity=''; t.style.transition=''; }, 260); }
}

function fmtDate(s) {
    if (!s) return { day: '--', mon: '--', full: 'Chưa xác định', iso: '' };
    var d = new Date(s);
    if (isNaN(d.getTime())) return { day: '--', mon: '--', full: 'Chưa xác định', iso: '' };
    return {
...
function renderApptCard(a, canCancel) {
    ...
    var ds  = a._displayStatus || a.status;   // display status (đã chuẩn hoá)
    var bg  = BADGE_MAP[ds] || ['bc', a.statusDisplay || a.status];

    // Cancel / rebook / payment buttons
    var cancelBtn  = '';
    var rebookBtn  = '';
    var paymentHtml = '';

    if (canCancel && (a.status === 'ChoXacNhan' || a.status === 'DaXacNhan'))
        cancelBtn = '<button class="btn-sm btn-danger" onclick="cancelAppt('+a.id+')">Hu\u1EF7 l\u1ECBch</button>';
    if (ds === 'HoanThanh' || a.status === 'DaHuy')
        rebookBtn = '<a class="btn-sm btn-primary-sm" href="/Booking">\u0110\u1EB7t l\u1EA1i</a>';

    // Thanh toán — chỉ hiện khi Hoàn thành và chưa thanh toán thành công
    if (ds === 'HoanThanh') {
        var paid = a.paymentStatus === 'Success';
        if (paid) {
            // Đã thanh toán → hiện badge xanh
            var method = a.paymentMethod === 'vnpay' ? 'VNPay' : 'tiền mặt';
            paymentHtml = '<div class="payment-info">'
                + '<span class="badge badge-paid">\uD83D\uDCB0 \u0110\u00E3 thanh to\u00E1n qua ' + method + '</span>'
                + '</div>';
        } else {
            // Chưa thanh toán → hiện nút VNPay + ghi chú
            paymentHtml = '<div class="payment-info">'
                + '<span class="badge badge-cash-pending" style="margin-right:8px;">\u23F3 Ch\u01B0a thanh to\u00E1n</span>'
                + '<a class="btn-pay-now" href="/Payment/PayNow/' + a.id + '">'
                + '\uD83D\uDCB3 Thanh to\u00E1n qua VNPay</a>'
                + '</div>';
        }
    }

    var actionsHtml = '';
    if (cancelBtn || rebookBtn) {
    ...
    return '<div class="appt-card" id="ac-'+a.id+'">'
        ...
        + '<div class="appt-body">'
            + '<div class="appt-doc">'+(a.doctorName||'')
...
function initSchedule() {
    ...
    // Chuẩn hoá: lịch DaXacNhan/DaDen/DangKham mà đã thanh toán thành công
    // → hiển thị như HoanThanh (bác sĩ đã ghi chẩn đoán nhưng staff chưa đổi trạng thái)
    allAppts.forEach(function(a) {
        if ((a.status === 'DaXacNhan' || a.status === 'DaDen' || a.status === 'DangKham')
            && a.paymentStatus === 'Success') {
            a._displayStatus = 'HoanThanh';
        } else {
            a._displayStatus = a.status;
        }
    });

    var up   = allAppts.filter(function(a){ return UPCOMING_ST.indexOf(a._displayStatus) >= 0; })
                       .sort(function(a,b){ return new Date(b.bookedAt)-new Date(a.bookedAt); });
    var hist = allAppts.filter(function(a){ return UPCOMING_ST.indexOf(a._displayStatus) < 0; })
                       .sort(function(a,b){ return new Date(b.workDate)-new Date(a.workDate); });
    var done = allAppts.filter(function(a){ return a._displayStatus==='HoanThanh'; });
...
function cancelAppt(id) {
    _cancelId = id;
    document.querySelectorAll('.cancel-reason').forEach(function(el){ el.classList.remove('selected'); });
    document.getElementById('cancelOtherText').style.display = 'none';
    document.getElementById('cancelOtherText').value = '';
    var btn = document.getElementById('btnDoCancel');
    btn.disabled = false; btn.textContent = 'X\u00E1c nh\u1EADn hu\u1EF7';
    document.getElementById('cancelModal').classList.add('open');
}
function closeCancelModal() { document.getElementById('cancelModal').classList.remove('open'); }
function pickReason(el) {
    document.querySelectorAll('.cancel-reason').forEach(function(o){ o.classList.remove('selected'); });
    el.classList.add('selected');
    document.getElementById('cancelOtherText').style.display = el.dataset.val === 'other' ? 'block' : 'none';
}
function doCancel() {
    var selected = document.querySelector('.cancel-reason.selected');
    if (!selected) { showToast('Vui l\u00F2ng ch\u1ECDn l\u00FD do hu\u1EF7.', false); return; }
    var reason = selected.dataset.val === 'other'
        ? (document.getElementById('cancelOtherText').value.trim() || 'L\u00FD do kh\u00E1c')
    ...
    .then(function(d){
        ...
        if (d.success) {
            ...
            var card = document.getElementById('ac-' + _cancelId);
            if (card) { card.style.opacity='0'; card.style.transition='opacity .3s'; }
            setTimeout(function(){ reloadAppointments(); }, 350);
        } else {
            showToast(d.message || 'Kh\u00F4ng th\u1EC3 hu\u1EF7.', false);
            btn.disabled = false; btn.textContent = 'X\u00E1c nh\u1EADn hu\u1EF7';
        }
    }).catch(function(){ closeCancelModal(); showToast('L\u1ED7i k\u1EBFt n\u1ED1i.', false); });
}
...
// \u2500\u2500 Kh\u1EDFi \u0111\u1ED9ng \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
initSchedule();
// Lu\u00F4n fetch d\u1EEF li\u1EC7u m\u1EDBi nh\u1EA5t ngay khi load \u2014 b\u1ED5 sung l\u1ECBch m\u1EDBi \u0111\u1EB7t v\u00E0 c\u1EADp nh\u1EADt tr\u1EA1ng th\u00E1i thanh to\u00E1n
reloadAppointments();
...
// Kích hoạt toast từ TempData (sau redirect đặt lịch thành công)
(function() {
    if (window._toastOnLoad) {
        // Delay nhỏ để DOM render xong
        setTimeout(function() {
            showToast(window._toastOnLoad.msg, window._toastOnLoad.ok);
        }, 400);
    }
})();
